...
def load_cleaned_shelter_data():
    """讀取清理後的避難收容處所資料"""
    filepath = 'outputs/shelter_shelters_cleaned.csv'
    
    try:
        df = pd.read_csv(filepath, encoding='utf-8-sig')
        print(f"載入 {len(df)} 筆清理後的避難收容處所資料")
        return df
    except Exception as e:
        print(f"讀取清理後避難所資料失敗: {e}")
        return pd.DataFrame()
...
def create_cleaned_map():
    """建立清理後的整合地圖"""
    print("="*70)
    print("建立清理後的 AQI 與避難收容處所整合地圖")
    print("="*70)
    
    # 獲取資料
    aqi_data = fetch_aqi_data()
    shelter_df = load_cleaned_shelter_data()
    ...
    print(f"清理後避難收容處所: {len(shelter_df)} 個")
    ...
    # ===== 圖層 B: 清理後避難收容處所（使用 MarkerCluster 優化）=====
    # 室內避難所叢集
    indoor_cluster = MarkerCluster(name='避難所-室內 (清理後)').add_to(m)
    # 室外避難所叢集  
    outdoor_cluster = MarkerCluster(name='避難所-室外 (清理後)').add_to(m)
    ...
    <div style="position: fixed; 
        ...
        <p style="margin: 5px 0;"><span style="display: inline-block; width: 12px; height: 12px; background-color: blue; border-radius: 2px; margin-right: 8px;"></span>避難所-室內 (清理後)</p>
        <p style="margin: 5px 0;"><span style="display: inline-block; width: 12px; height: 12px; background-color: green; border-radius: 2px; margin-right: 8px;"></span>避難所-室外 (清理後)</p>
    </div>
    '''
    m.get_root().html.add_child(folium.Element(legend_html))
    
    # ===== 儲存地圖 =====
    output_path = 'outputs/aqi_shelter_integrated_map_cleaned.html'
    m.save(output_path)
