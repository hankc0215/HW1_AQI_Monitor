...
def analyze_shelter_aqi():
    """分析避難所與最近 AQI 測站"""
    print("="*70)
    print("避難所最近測站分析與風險評估")
    print("="*70)
    ...
    print(f"避難收容處所數量: {len(shelter_df)}")
    ...
    # 保存結果
    output_path = 'outputs/shelter_aqi_analysis.csv'
    ...
    if injection_applied and injected_station:
        ...
        print(f"\n情境注入影響: {len(affected_shelters)} 個避難所受到 {injected_station.get('sitename')} 測站影響")
