...
def get_reverse_geocode(lat: float, lon: float) -> Dict:
    ...
    try:
        ...
        headers = {
            'User-Agent': 'AQI Shelter Location Validator (Cleaned)'
        }
...
def validate_cleaned_shelter_locations():
    """驗證清理後避難所位置"""
    print("="*70)
    print("清理後避難所位置驗證")
    print("="*70)
    ...
    print(f"開始驗證 {len(shelter_df)} 個清理後避難所位置...")
    ...
    for idx, row in shelter_df.iterrows():
        try:
            ...
            # 進度顯示
            if (idx + 1) % 50 == 0:
                print(f"已處理 {idx + 1}/{len(shelter_df)} 個清理後避難所...")
                
        except Exception as e:
            print(f"處理清理後避難所 {idx} 時發生錯誤: {e}")
            continue
    ...
    # 輸出統計
    print(f"\n清理後驗證完成!")
    print(f"總共驗證: {len(results_df)} 個清理後避難所")
    ...
    print(f"\n發現位置問題的清理後避難所: {len(problematic)} 個")
    ...
    # 保存結果
    output_path = 'outputs/shelter_location_validation_cleaned.csv'
    results_df.to_csv(output_path, index=False, encoding='utf-8-sig')
    print(f"\n清理後驗證結果已儲存至: {output_path}")
    
    # 生成問題清單
    if len(problematic) > 0:
        problem_path = 'outputs/shelter_location_issues_cleaned.csv'
        problematic.to_csv(problem_path, index=False, encoding='utf-8-sig')
        print(f"清理後問題清單已儲存至: {problem_path}")
    
    return results_df

if __name__ == "__main__":
    validate_cleaned_shelter_locations()
