...
def analyze_cleaned_shelter_aqi():
    """分析清理後避難所與最近 AQI 測站"""
    print("="*70)
    print("清理後避難所最近測站分析與風險評估")
    print("="*70)
    ...
    print(f"清理後避難收容處所數量: {len(shelter_df)}")
    ...
    # 保存結果
    output_path = 'outputs/shelter_aqi_analysis_cleaned.csv'
    results_df.to_csv(output_path, index=False, encoding='utf-8-sig')
    print(f"\n清理後分析結果已儲存至: {output_path}")
    ...
    if injection_applied and injected_station:
        ...
        print(f"\n情境注入影響: {len(affected_shelters)} 個清理後避難所受到 {injected_station.get('sitename')} 測站影響")
