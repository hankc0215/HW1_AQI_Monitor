...
"""
重新生成最終清理後的整合地圖
使用最終清理後的避難所資料重新建立 AQI 與避難所整合地圖
"""
...
def create_final_cleaned_map():
    """建立最終清理後的整合地圖"""
    print("="*70)
    print("建立最終清理後的 AQI 與避難收容處所整合地圖")
    print("="*70)
    ...
    print(f"最終清理後避難收容處所: {len(shelter_df)} 個")
    ...
    # ===== 圖層 B: 最終清理後避難收容處所（使用 MarkerCluster 優化）=====
    # 室內避難所叢集
    indoor_cluster = MarkerCluster(name='避難所-室內 (最終清理後)').add_to(m)
    # 室外避難所叢集  
    outdoor_cluster = MarkerCluster(name='避難所-室外 (最終清理後)').add_to(m)
    ...
    for idx, row in shelter_df.iterrows():
        try:
            ...
            if not is_valid:
                validation_failed += 1
                print(f"座標驗證失敗: {row.iloc[6]} - {reason}")
                continue
    ...
    <div style="position: fixed; 
        ...
        <p style="margin: 5px 0;"><span style="display: inline-block; width: 12px; height: 12px; background-color: blue; border-radius: 2px; margin-right: 8px;"></span>避難所-室內 (最終清理後)</p>
        <p style="margin: 5px 0;"><span style="display: inline-block; width: 12px; height: 12px; background-color: green; border-radius: 2px; margin-right: 8px;"></span>避難所-室外 (最終清理後)</p>
    </div>
    '''
    m.get_root().html.add_child(folium.Element(legend_html))
    
    # ===== 儲存地圖 =====
    output_path = 'outputs/aqi_shelter_integrated_map_final.html'
    m.save(output_path)
    ...
    if validation_failed == 0:
        print("\n[驗證通過] 所有最終清理後避難所座標均在合理範圍內")
    else:
        print(f"\n[警告] 發現 {validation_failed} 個座標異常的避難所")
    
    return output_path

if __name__ == "__main__":
    create_final_cleaned_map()
